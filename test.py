...
if opt.multi:
    ...
    image_datasets = {x: datasets.ImageFolder( os.path.join(data_dir,x) ,data_transforms) for x in ['gallery_satellite','query_street']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
...
query_path = image_datasets['query_street'].imgs
...
with torch.no_grad():
    ...
    query_feature = extract_feature(model,dataloaders['query_street'])
